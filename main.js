...
const buttons = document.getElementById("buttons-across");
const screen_bottom = document.getElementById("screen-bottom");

const image = document.getElementById("img-pokemon");
const name_pokemon = document.getElementById("name-pokemon");
...
const MODAL_SCREEN = () => {
  if (!btn_s.disabled) {
    buttons.classList.toggle("display-n");
...
//----------------------------------------
function ERROR() {
  image.innerHTML = `<img src="./images/error.webp" alt="ERROR" style="filter: grayscale(90%);">`;
  name_pokemon.value = "No existe";
  resetUI();
}
